...
h_x = zeros(1,69,10);%raw data 10 hand
h_y = zeros(1,69,10);
...
registured_hand_x_list = zeros(1,69,9);
registured_hand_y_list = zeros(1,69,9);

mean_9_hand_x = zeros(1,69);
mean_9_hand_y = zeros(1,69);
...
pre_mean_10_hand_x = zeros(1,69);
pre_mean_10_hand_y = zeros(1,69);

for oo = 1:100
    
    for i = 2:10
        [registured_hand_x, registured_hand_y, ~, ~] = ICP(h_x(1,:,i), h_y(1,:,i),first_hand_x ,first_hand_y);
...
hold on;
for i = 1:9
    xc1 = registured_hand_x_list(1,:,i);
    yc1 = registured_hand_y_list(1,:,i);
    plot(xc1,yc1,'-*b');
end
hold off;
...
de_mean_10_hand_x = zeros(1,69,10); 
de_mean_10_hand_y = zeros(1,69,10); 
...
reshape_combined_de_mean_10_hand = reshape(combined_de_mean_10_hand,138,10);
...
subplot(2,2,1);
for i = -100:10:100
    contour = combined_mean_10_hand + reshape(i * column(:,1),69,2);
...
subplot(2,2,2);
for i = -100:10:100
    contour = combined_mean_10_hand + reshape(i * column(:,2),69,2);
    plot(contour(:, 1), contour(:, 2), 'b-');
    pause(0.1);
...
subplot(2,2,3);
for i = -100:10:100
    contour = combined_mean_10_hand + reshape(i * column(:,3),69,2);
    plot(contour(:, 1), contour(:, 2), 'b-');
    pause(0.1);
...
subplot(2,2,4);
for i = -100:10:100
    contour = combined_mean_10_hand + reshape(i * column(:,4),69,2);
    plot(contour(:, 1), contour(:, 2), 'b-');
    pause(0.1);
...
for i = 1:100
    ...
    contour = combined_mean_10_hand + reshape(eigen * b, mean_shape_points_num, 2);
    contour_x = contour(:, 1)';
    contour_y = contour(:, 2)';
    new_point_x = xy(:, 1)';
    new_point_y = xy(:, 2)';
    
    [shape_x, shape_y, T, r] = ICP(contour_x, contour_y, new_point_x, new_point_y);
...
%     [calced_matrix_x, calced_matrix_y] = distance_match(contour_x, contour_y, new_point_x, new_point_y);
    [calced_matrix_x, calced_matrix_y] = bestmatch(contour, xy);
...
function [processed_point_x, processed_point_y, T, r] = ICP (initial_point_x, initial_point_y, target_point_x, target_point_y)
